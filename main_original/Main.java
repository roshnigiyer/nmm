...
public class Main {
  ...
  public static final int shuffleSeed = 42; // to do: make this totally random..not set to 42
  ...
  public static double calcObj(double[] R, double[][] theta, List<Integer> allEdges) {
    ...
    for (int _e = 0; _e < 2*E*tp; _e++) {
      ...
      double lw = 1.0;
      double dz = l1dist(theta[s], theta[t]);
      double dr = R[s]-R[t];
  ...
  public static void update(double[] R, double[][] theta, int i, int j, boolean label, double[][] grad) {
     ...
     */ 
    double lw = 1.0;
    double dz = l1dist(theta[i], theta[j]);
    double dr = R[i]-R[j];

    // Roshni introduced ///

    // p(e_ij | d(z_i, z_j))
//    double A_hat = Math.random();
//    double A = Math.exp(A_hat);
//    double B = Math.random();
//    double p_hom = 1 / (1 + Math.exp(A * dz + B));
//
//    // p(e_ij | d(r_i, r_j))
//    double C_hat = Math.random();
//    double C = Math.exp(C_hat);
//    double D = Math.random();
//    double numerator = Math.exp(C * dr + D);
//    double p_rank = numerator / (1 + numerator);
	...
	grad[0][k] += g * lw * useNsw; // deriv(logsigmoid(p_ij)) / deriv(z_i)
  ...
  public static void runSGD(double[] R, double[][] theta, double convergenceTol) {
	...
	// stepsize
        // TO DO: change this to descent not ascent e.g., theta[s][k] -= & R[s] -=
	double thisStepSize = sgdStepSize * (1.0 - count / MAX_EDGES);
	for (int k = 0; k < K+1; k++) {
	  if (k < K) {
